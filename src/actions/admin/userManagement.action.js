...
export const updateUserByID = (userData, userId) => {
  ...
  return async (dispatch) => {
    try {
      ...
      if (!accessToken) {
        return dispatch({
          type: UPDATE_CUSTOMER_BY_ID_ERROR,
          error: "Access token is missing",
        });
      }
      dispatch({ type: UPDATE_CUSTOMER_BY_ID_PENDING });
      ...
      dispatch({ type: UPDATE_CUSTOMER_BY_ID_SUCCESS, data: user });
    } catch (error) {
      await dispatch({
        type: UPDATE_CUSTOMER_BY_ID_ERROR,
...
export const deleteUserByID = (userID) => {
  return async (dispatch) => {
    try {
      ...
      dispatch({ type: DELETE_USER_BY_ID_PENDING });
...
export const onCustomerSearch = (currentPage, itemPerPage, searchText) => {
  return async (dispatch) => {
    try {
      ...
      await dispatch({
        type: ON_SEARCH_CUSTOMER_PENDING,
      });
      ...
      const data = dataObj.data;
      return dispatch({
        type: ON_SEARCH_CUSTOMER_SUCCESS,
        data: data,
        totalCount: dataObj.totalItems,
        currentPage: dataObj.currentPage,
        limit: dataObj.limit,
...
export const onCustomerFilter = (currentPage, itemPerPage, filterValue) => {
  return async (dispatch) => {
    try {
      ...
      await dispatch({
        type: ON_FILTER_CUSTOMER_PENDING,
      });
      const response = await fetch(
      ...
      const data = dataObj.data;
      return dispatch({
        type: ON_FILTER_CUSTOMER_SUCCESS,
        data: data,
        totalCount: dataObj.totalItems,
        currentPage: dataObj.currentPage,
        limit: dataObj.limit,
