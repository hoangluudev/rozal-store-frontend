...
import { MoreVert, PaidOutlined } from "@mui/icons-material";
import { convertToCurrency } from "../../../../utils/formatting";
import { useDashboardApi } from "@/hooks/api";

export const TotalSales = () => {
  const { totalSales } = useDashboardApi().state;
  ...
  const selectedData = totalSales
    ? totalSales[datasetMapping[selectDataset]]
    : {};
        ...
        <Stack flexDirection={"column"} justifyContent={"center"} gap={2}>
          ...
          >
            <Stack flexDirection={"row"} alignItems={"center"} columnGap={1}>
              <PaidOutlined sx={{ color: textColor }} fontSize="large" />
              <Typography
                sx={{
                  fontSize: 16,
            ...
            >
              {datasetList.map((option) => (
                <MenuItem
                  ...
                  style={{
                    backgroundColor:
                      option.value === selectDataset ? "#b39ddb" : "inherit",
                  }}
                >
                  {option.label}
                </MenuItem>
